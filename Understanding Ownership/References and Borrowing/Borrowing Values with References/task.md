...
> Note: The opposite of referencing by using `&` is _dereferencing_, which is accomplished with the dereference operator, `*`. We’ll see some uses of the dereference operator in the [Common Collections](course://Common+Collections) section and discuss details of dereferencing in the [Smart Pointers](course://Standard+Library+Types/Smart+Pointers) lesson.
